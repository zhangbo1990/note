...
5. 创建多少个线程合适
   1. 度量性能的标准-延时和吞吐量
   2. 延时，时间纬度，请求收到到响应的时间，越短性能越好。
   3. 吞吐量，空间纬度，单位时间内处理的请求数量
   4. 性能提升，延时越小，吞吐量越大，性能越好。
   5. 提升性能两个方向，优化算法和将硬件性能发挥到极致，后者和并发息息相关。
   6. 两个方面，增加线程数（提高io和cpu的综合利用率），增加cpu核数。
   7. 那么设置多少个线程合适呢？两种类型设置的线程数不同。
   8. cpu密集型，理论上 最佳线程数=cpu核数 工程上会设置为 线程数=cpu核数+1，为的是其他原因阻塞，可以顶替。
   9. io密集型，最佳线程数=cpu核数 * [（io 耗时 / CPU耗时）+1]
6. 为什么局部变量是线程安全的
   1. 局部变量与方法生命周期同步，都存放在自己的调用栈中，并且每个线程都有自己独立的一份调用栈，所以互不干扰，这也是堆和栈的区别，如果想让对象跨越方法就要在堆中创建对象。（线程封闭也是这个思想）
7. java语言里面向对象思想，可以让并发实现起来更简单
   1. 封装共享变量，类比现实场景收门票，观众只能从检票口进入，如果还有其他入口，检票口就形同虚设了，这与封住的思想高度吻合，将共享变量封装，只能通过对外的暴露的公共方法访问，就能更容易的保证线程安全。或者将不可变量设置成final。
   2. 识别共享变量之间的约束条件，表现在代码中一般是if，if判断条件成立和执行直接一定要保证原子性，保证执行时条件也是成立的。
   3. 制定并发访问策略，避免共享变量？？，不变模式？？管程和其他同步工具。
8. 小结
   1. 锁是私有的，不可变，不可重用的 
```java
   // 普通对象锁
